...
TCgdi::TCgdi(void)
{
	...
	FhGrayPen			= CreatePen(PS_SOLID, 1, colors[gray]);
	...
	FhDarkGreenPen		= CreatePen(PS_SOLID, 1, colors[green]);
	FhLightBluePen		= CreatePen(PS_SOLID, 1, colors[light_blue]);
	FhLightGrayPen		= CreatePen(PS_SOLID, 1, colors[light_gray]);
	...
	FhGrayBrush			= CreateSolidBrush(colors[gray]);
