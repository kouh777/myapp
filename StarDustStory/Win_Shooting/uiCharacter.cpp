...
#define TRIMMING__IMAGE_RBX 600	// 
#define TRIMMING__IMAGE_RBY 800	// 

//----------------------------------------------
TuiCharacter::TuiCharacter( TsceneGame *game, const Vector2D &pos, const Vector2D &velocity)
	:TobjUi(
	game,
	pos,						// position
	2.0,						// radius
...
//----------------------------------------------
TuiCharacter::~TuiCharacter(void)
{
}

//----------------------------------------------
BOOL TuiCharacter::Update(double time_elapsed)
{
...
void TuiCharacter::Render( void )
{
	...
	// 画像を表示する座標
	FpGame->FpCharacterSeiya->RenderEx(
							&srcRec,
							pos,																// DrawPosition
...
void TuiCharacter::RenderCgdi()
{
	...
	Cgdi().BluePen();
	Cgdi().BlueBrush();
	Cgdi().Circle(vec[0], FdBoundingRadius);
}
